...
static uint8_t split_central_sensor_notify_func(struct bt_conn *conn,
    ...
    int err;
    const struct sensors_item_cfg *item = &sensors[sensor_event->sensor_number];
    ...
    struct zmk_sensor_event ev = {.sensor_index = sensor_event->sensor_number,
                                  .channel_data_size = 1,
                                  .channel_data = {(struct zmk_sensor_channel_data){
                                      .value = sensor_event->value, .channel = item->trigger.chan}},
                                  .timestamp = k_uptime_get()};
